...
bool NinjaTrap::isSame(ClapTrap const *attacker, ClapTrap const *target) const
{
	return ((attacker == target) ? true : false);
}

void NinjaTrap::ninjaShoebox(SuperTrap const & target) const {
	std::cout << std::endl;
	if (isSame(this, &target))
		std::cout << "Let's practice harakiri!\n";
	else if (target.getModel() == this->getModel())
		std::cout << "Oh, another SuperTrap! Are you as cool as me?\n";
	else
